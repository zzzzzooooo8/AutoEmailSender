...
## 3. 发送安全策略
- 系统只有一个全局发送模式 `mail_delivery_mode`
- 默认是 `dry_run`
- 用户在工作区点击“批准并发送”或“批准并排程”时，会把当时的全局模式快照到任务上
- 已批准或已排程任务不会因为之后再切换全局模式而被重写
- 只有 `live` 发出的邮件才参与 IMAP 回复关联
...
- `sent`：`dry_run` 和 `live` 都会进入这个状态，具体模式看 `delivery_mode`
- `reply_detected`：只会出现在 `live` 发送并检测到回复后
