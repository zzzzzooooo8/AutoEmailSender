...
- **网页抓取 (Web Scraping)**: Playwright + Markdown 转换 + LLM-Assisted Scraper
- **大模型集成 (LLM)**: 任意兼容 OpenAI 格式的 API
...
│   │   ├── agents/             # LLM 辅助抓取、匹配与写作逻辑
│   │   │   ├── crawler.py      # 入口页下载、Markdown 转换、路由判断与详情抽取
...
### 6.3 模型辅助爬虫 (`backend/app/agents/crawler.py`)
**输入**
- 用户提供的目标院校教师名录链接、导师列表链接，或其他明确可作为入口的种子 URL

**处理流程**
1. 以用户提供的入口链接作为起点，而不是依赖搜索引擎全网搜索。
2. 使用 Playwright 下载网页内容，必要时等待页面渲染完成，但不依赖自主浏览式 Agent 决策。
3. 将网页正文转换为 Markdown，作为后续 LLM 判断与提取的统一输入格式。
4. 先由轻量 LLM 路由器判断当前页面类型：`列表页`、`分页列表页`、`导师详情页` 或 `无关页`。
5. 若为列表页或分页列表页，则由传统代码负责收集候选详情链接、处理翻页、去重与调度下一批待抓取 URL。
6. 若为导师详情页，则由高质量提取模型将 Markdown 转换为严格 JSON，至少提取字段：`姓名`、`邮箱`、`职称`、`学校`、`院系`、`研究方向`、`近期论文`、`主页链接`、`来源页面`。
7. 提取结果必须经过结构校验后再持久化，并记录爬取状态、页面类型判断结果与跳过原因。
...
- 页面导航、链接收集、翻页和去重由传统代码控制，不把抓取流程完全交给 LLM 自主探索。
- LLM 在抓取模块中的职责限定为“页面路由判断”和“详情页结构化提取”，不承担开放式全网搜索。
...
- 抓取策略采用“模型辅助爬虫”而不是开放式自治 Agent：网页访问与翻页由代码调度，LLM 负责页面判断与详情提取。
...
8. **抓取职责边界清晰**: 页面访问、链接调度、翻页和去重使用确定性代码；LLM 仅用于页面分类与结构化提取。
9. **结构化输出必须校验**: 详情页提取结果必须要求严格 JSON，并在入库前做 Pydantic 或等价校验。
10. **错误处理**: Playwright 超时、页面结构变化、LLM API 错误、SMTP / IMAP 失败都不能导致主服务崩溃。
11. **React 最佳实践**: 使用函数式组件和 React Hooks，优先通过 React Context 管理全局配置，避免不必要地引入 Redux。
12. **CORS 配置**: 正确配置 FastAPI 中间件，以支持 Vite 开发环境的跨域请求。
13. **本地优先**: 文件上传、简历解析、配置存储、日志记录都应优先采用本地方案，不以云服务为前提。
