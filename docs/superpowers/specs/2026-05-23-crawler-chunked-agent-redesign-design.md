...
| 参数 | 默认值 | 下限 / 上限 | 说明 |
| ---- | ---- | ---- | ---- |
| `target_tokens` | 2,000 | 可调范围 1,600-2,200 | 目标大小，通常希望一个 chunk 内候选数低于 10。 |
...
- `too_many_candidates`：本 chunk 候选超过 10 个，需要拆分。
- `uncertain`：内容疑似相关，但模型无法可靠判断，可由后端重试或拆分。

后端不能完全信任模型的 `has_more_candidates_in_chunk`。如果返回候选数刚好为 10，默认认为有截断风险，除非 chunk 足够小且模型明确 `has_more_candidates_in_chunk=false`。
...
- `candidates.length == 10` 且 `has_more_candidates_in_chunk = true`。
...
输入包含 `chunk_id`、`chunk_status`、`has_more_candidates_in_chunk` 和候选列表。后端执行：

1. 校验 chunk 是否仍处于 `processing`。
2. 校验候选数量不超过 10。
3. 保存、合并或拒绝候选。
4. 根据候选数量和 `has_more_candidates_in_chunk` 决定 chunk 状态。
5. 必要时触发自动拆分。
6. 返回短反馈和下一步建议。
...
### 风险：模型误判 `has_more_candidates_in_chunk`

后端不能完全信任该字段。候选数刚好为 10 时采取保守拆分策略。
